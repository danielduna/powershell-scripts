...
# Log File Path.
$logFilePath = "C:\Temp\LogFile_$(Get-Date -Format 'yyyyddMM').log"
...
# Function - Connect SPO Using Managed Identity
...
function Main {
    # Define Path To CSV File In Blob Storage.
    $storageAccountName = "sitescsv"
    $containerName = "files"
    $blobName = "Sites.csv"
    $csvFilePath = "C:\Temp\Sites.csv"

    # Retrieve CSV From Blob Storage
    $context = New-AzStorageContext -StorageAccountName $storageAccountName -UseManagedIdentity
    Get-AzStorageBlobContent -Container $containerName -Blob $blobName -Destination $csvFilePath -Context $context
    ...
    $logBlobName = "Logs/LogFile_$(Get-Date -Format 'yyyyddMM').log"
    Set-AzStorageBlobContent -File $logFilePath -Container $containerName -Blob $logBlobName -Context $context
}
